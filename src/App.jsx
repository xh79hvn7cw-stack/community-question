...
const S = {
  ...
  // share button
  shareBtn:  (loading) => ({ background: loading ? "rgba(255,255,255,0.05)" : "transparent", border: "1px solid rgba(232,197,71,0.3)", borderRadius: 10, padding: "13px 0", width: "100%", marginTop: 10, fontSize: 13, fontWeight: 700, color: loading ? "#4B5563" : "#E8C547", cursor: loading ? "not-allowed" : "pointer", fontFamily: "'Syne', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }),

  // share card (rendered offscreen then captured)
...
export default function App() {
  ...
  // ── Question card ───────────────────────────────────────────────────────
  const QCard = ({ q }) => (
    <div style={S.qCard} onClick={() => { setSelectedQId(q.id); setView("question"); }}
    ...
    <>
      ...
      {selectedQ && (
        <div ref={shareCardRef} style={S.shareCard}>
          ...
          <div style={S.scStats}>
            <div style={S.scStat}>
              <div style={S.scStatV(false)}>{selectedQ.votes.toLocaleString()}</div>
              <div style={S.scStatL}>people asking</div>
            </div>
            <div style={S.scStat}>
              <div style={S.scStatV(true)}>{selectedQ.daysUnanswered}</div>
              <div style={S.scStatL}>days ignored</div>
            </div>
            <div style={S.scStatLast}>
              <div style={S.scStatV(true)}>{selectedQ.timesDeflected}×</div>
              <div style={S.scStatL}>deflected</div>
            </div>
          </div>
